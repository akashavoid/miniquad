// Macroquad Context Factory - Transforms gl.js into encapsulated context

// Global plugin registry - plugins register once, contexts copy from here
if (!window._macroquad_global_plugins) {
  window._macroquad_global_plugins = [];
}

// Global plugin registration function - called by IIFE plugins
window.miniquad_add_plugin = function(plugin) {
  window._macroquad_global_plugins.push(plugin);
};

function createMacroquadContext(canvas) {
  if (!canvas) {
    throw new Error("createMacroquadContext requires a canvas element");
  }


  const version = 2;

  // canvas parameter used instead of querySelector
  var gl;

  var clipboard = null;

  // Copy plugins from global registry (each context gets own copy)
  var plugins = [...window._macroquad_global_plugins];
  var wasm_memory;
  var wasm_exports;
  ...
  function init_webgl(version) {
      console.log("Initializing WebGL " + version);
      if (version == 1) {
          ...
          run_animation_loop: function (blocking) {
              ...
              var checkFocus = function () {
                  ...
                  if (lastFocus != hasFocus) {
                      // Defensive: only call if focus() export exists
                      if (wasm_exports && wasm_exports.focus) {
  ...
  function register_plugins(plugins, context) {
      if (plugins == undefined)
          return;

      for (var i = 0; i < plugins.length; i++) {
          if (plugins[i].register_plugin != undefined && plugins[i].register_plugin != null) {
              plugins[i].register_plugin(context, importObject);
          }
      }
  }
  ...
  // Note: Plugins register to global window._macroquad_global_plugins
  // This context copied them at creation time (line 27)
  ...
  function load(wasm_path) {
      ...
      // Create context object with core utilities
      // Plugins will use this instead of globals
      const context = {
        wasm_memory,         // Set after WASM instantiation
        wasm_exports,        // Set after WASM instantiation
        UTF8ToString,
        getArray,
        stringToUTF8,
        // Placeholder for sapp-jsutils to populate
        js_object: null,
        consume_js_object: null,
        get_js_object: null,
      };

      register_plugins(plugins, context);
          ...
          WebAssembly.compileStreaming(req)
              ...
              .then(
                  obj => {
                      ...
                      // Update context with WASM exports (plugins may have stored reference)
                      context.wasm_memory = wasm_memory;
                      context.wasm_exports = wasm_exports;
  ...
  return {
    load,
    destroy() {
      console.log("🧹 Destroying Macroquad context...");

      // Cancel animation loop
      if (animation_frame_timeout) {
        cancelAnimationFrame(animation_frame_timeout);
      ...
      console.log("✅ Macroquad context destroyed");
    }
  };
}
...
if (typeof window !== "undefined") {
  window.createMacroquadContext = createMacroquadContext;
}

if (typeof module !== "undefined" && module.exports) {
  module.exports = createMacroquadContext;
}